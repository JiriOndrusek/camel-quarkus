...
public class CamelAiToolProvider implements ToolProvider {
    ...
    // Invoked reflectively to keep camel-langchain4j-agent an optional dependency — this support module
    // serves all langchain4j extensions, not just ai-tool, so a hard compile-time reference would force
    // the agent jar onto every user's classpath.
    private static ToolSpecification toToolSpecification(AiToolSpec spec) {
        try {
            return (ToolSpecification) TO_TOOL_SPECIFICATION.invoke(null, spec);
        } catch (ReflectiveOperationException e) {
            throw new RuntimeException("Failed to convert AiToolSpec to ToolSpecification", e);
        }
    }

    private static Method resolveToToolSpecification() {
        try {
            Class<?> clazz = Class.forName("org.apache.camel.component.langchain4j.agent.AiToolSpecToLangChain4j");
            return clazz.getMethod("toToolSpecification", AiToolSpec.class);
        } catch (ClassNotFoundException | NoSuchMethodException e) {
            throw new RuntimeException(
                    "camel-langchain4j-agent is required on the classpath when using CamelAiToolProvider", e);
        }
    }
