...
The ingestion engine moved to Apache Camel as the `camel-langchain4j-ingest` component, and the extension now drives that component instead of carrying its own implementation.
The user experience is unchanged — the same `quarkus.camel.langchain4j.ingest.*` properties, the same `@Ingest` builder API, the same route ids, error messages, `CamelIngestDocumentId` header and deduplication semantics.
One behavioural detail changed with the move:

=== Segment metadata keys are now camel_ingest_*

Segments were written with `camel_quarkus_pipeline` and `camel_quarkus_document_id` metadata; the component writes the runtime-neutral `camel_ingest_pipeline` and `camel_ingest_document_id` instead.
