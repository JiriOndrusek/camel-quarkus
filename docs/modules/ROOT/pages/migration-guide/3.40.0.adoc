...
These are the key names of the `camel-langchain4j-ingest` component introduced in Camel 4.23.0, which this extension delegates its engine to — stores written from 3.40.0 on are compatible with that engine as-is, so users of this LTS stream will not face a second key change.
