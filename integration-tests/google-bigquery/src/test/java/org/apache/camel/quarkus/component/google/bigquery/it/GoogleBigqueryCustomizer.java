...
public class GoogleBigqueryCustomizer implements GoogleTestEnvCustomizer {
    ...
    public void customize(GoogleCloudContext envContext) {
        ...
        try {
            ...
            //for mocked run do not generate suffixes
            boolean generateSuffixes = !envContext.isUsingMockBackend();
            //if env property `GENERATE_SUFFIXES` is set, use it
            if (gs != null) {
                generateSuffixes = Boolean.parseBoolean(gs);
            }
            // ------------------ generate names ----------------
