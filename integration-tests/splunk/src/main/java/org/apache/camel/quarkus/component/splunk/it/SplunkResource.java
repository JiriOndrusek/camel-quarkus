...
public class SplunkResource {
    ...
    public List realtime(String search) throws Exception {
        ...
        String url = String.format(
                "splunk://realtime?scheme=http&port=%d&delay=500&initEarliestTime=rt-30s&latestTime=RAW(rt+30s)&search="
                        + search,
                port);
    ...
    public void startPolling(String search) {
        ...
        Executors.newSingleThreadExecutor().execute(() -> {
            String url = String.format(
                    "splunk://realtime?scheme=http&port=%d&delay=500&initEarliestTime=rt-10s&latestTime=RAW(rt+40s)&search="
                            + search,
                    port);
