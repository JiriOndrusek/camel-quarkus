...
public class GooglePubsubResource {
    ...
    @GET
    @Produces(MediaType.TEXT_PLAIN)
    public String mockReceive(@QueryParam("reset") boolean reset, @PathParam("mockName") String mockName)
            throws Exception {
    ...
    @Path("setFail/")
    @POST
    public Response mockReceive(boolean fail)
            throws Exception {
    ...
    @Path("/isConsumerStopped")
    @GET
    @Produces(MediaType.TEXT_PLAIN)
    public boolean isConsumerStopped() throws Exception {
        return stopResult != null && stopResult.isDone();
    }
}
