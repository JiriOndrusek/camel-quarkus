...
public class SpringRabbitmqResource {
    ...
    @POST
    @Produces(MediaType.TEXT_PLAIN)
    public String getFromDirect(@QueryParam(QUERY_DIRECT) String direct, @QueryParam(QUERY_TIMEOUT) int timeout) {
        final String message = timeout > 0 ? consumerTemplate.receiveBody(direct, timeout, String.class)
