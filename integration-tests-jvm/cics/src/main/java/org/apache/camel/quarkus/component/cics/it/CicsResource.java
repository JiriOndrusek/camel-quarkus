...
public class CicsResource {
    ...
    @Path("/load/component/cics")
    @GET
    @Produces(MediaType.TEXT_PLAIN)
    public Response loadComponentSap() throws Exception {
        /* This is an autogenerated test */
        if (context.getComponent(COMPONENT_CICS) != null) {
            return Response.ok().build();
        }
        LOG.warnf("Could not load [%s] from the Camel context", COMPONENT_CICS);
        return Response.status(500, COMPONENT_CICS + " could not be loaded from the Camel context").build();
    }
}
