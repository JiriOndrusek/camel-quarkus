...
public class KafkaTestResource implements QuarkusTestResourceLifecycleManager {
    ...
    public KafkaContainerProperties start(Function<String, StrimziKafkaContainer> containerSupplier) {
        LOGGER.info(TestcontainersConfiguration.getInstance().toString());
    ...
    protected KafkaContainerProperties createProperties() {
        KafkaContainerProperties kcp = new KafkaContainerProperties()
                .withSslKeystoreLocation(KAFKA_KEYSTORE_FILE)
                .withSslKeystoreType(KAFKA_KEYSTORE_TYPE)
                .withSslTruststoreLocation(KAFKA_TRUSTSTORE_FILE)
                .withSslTruststoreType(KAFKA_KEYSTORE_TYPE);
        if (container != null) {
            kcp = kcp.withBootstrapServers(container.getBootstrapServers());
        }
