...
public class GoogleCloudContext {
    ...
    public <C> Optional<C> getResource(Class<C> type) {
        return closeables.stream().filter(c -> type.isInstance(c)).map(c -> type.cast(c)).findFirst();
    }
    ...
    public ArrayList<AutoCloseable> getCloseables() {
        return closeables;
    }
