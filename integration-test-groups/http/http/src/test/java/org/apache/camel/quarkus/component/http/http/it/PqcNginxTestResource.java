...
public class PqcNginxTestResource implements QuarkusTestResourceLifecycleManager {
    ...
    public Map<String, String> start() {
        try {
            ...
            // Get certificate files from well-known locations
            // Certificates are generated by @PQCCertificates annotation on the test class
            // and written to target/certs directory
            Path certDirPath = Path.of("target/certs");
            Path certFile = certDirPath.resolve(CERT_NAME + "-cert.pem");
            Path keyFile = certDirPath.resolve(CERT_NAME + "-key.pem");
            Path truststoreFile = certDirPath.resolve(CERT_NAME + "-truststore.p12");
