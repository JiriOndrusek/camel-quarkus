...
abstract class AbstractKeyVaultContextRefreshTest {

    private static final Logger LOG = Logger.getLogger(AbstractKeyVaultContextRefreshTest.class);

    abstract String generateSecretName();

    String getEnvNameForEventHubConnectionString() {
        return "AZURE_EVENT_HUBS_CONNECTION_STRING";
    }

    private String generateRefreshEvent(String secretName) {
        return "[{\n" +
                "  \"subject\": \"" + generateSecretName() + "-.*\",\n" +
                "  \"eventType\": \"Microsoft.KeyVault.SecretNewVersionCreated\"\n" +
                "}]";
    }

    void contextRefresh() {
        String secretName = generateSecretName();
        ...
        try {
            ...
            try (EventHubProducerClient client = new EventHubClientBuilder()
                    .connectionString(System.getenv(getEnvNameForEventHubConnectionString()))
                    .buildProducerClient()) {
            ...
            try {
                String connectionString = getEnvNameForEventHubConnectionString();
