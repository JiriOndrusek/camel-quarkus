...
public class PqcRecorder {
    ...
    public void registerBouncyCastlePQCProvider(List<String> cipherTransformations, ShutdownContext shutdownContext) {
        ...
        for (String transformation : cipherTransformations) {
            try {
                if (transformation.contains("KeyPairGenerator")) {
                    // For key pair generators
                    LOG.debugf("Making it explicit to the static analysis that KeyPairGenerator %s could be used at runtime",
                            transformation);
                    KeyPairGenerator.getInstance(transformation, provider);
                } else {
                    // For signature algorithms
                    LOG.debugf("Making it explicit to the static analysis that Signature %s could be used at runtime",
                            transformation);
                    Signature.getInstance(transformation, provider);
                }
            } catch (Exception e) {
                // The algorithm is not present, a runtime error will be reported as usual
                LOG.debugf("Could not instantiate %s: %s", transformation, e.getMessage());
            }
        }
