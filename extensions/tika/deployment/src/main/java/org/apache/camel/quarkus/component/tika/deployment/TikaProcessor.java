...
import org.apache.camel.quarkus.component.tika.TikaConfiguration;
...
class TikaProcessor {
    ...
    CamelRuntimeBeanBuildItem tikaComponent(BeanContainerBuildItem beanContainer, TikaRecorder recorder,
        ...
        return new CamelRuntimeBeanBuildItem("tika",
                TikaComponent.class.getName(),
                recorder.createTikaComponent(beanContainer.getValue(), tikaXmlConfiguration));
    }
    ...
    private static String generateTikaXmlConfiguration(Map<String, List<TikaParserParameter>> parserConfig) {
        ...
        for (Map.Entry<String, List<TikaParserParameter>> parserEntry : parserConfig.entrySet()) {
    ...
    private static String getParserParamType(String parserName, String paramName) {
        try {
            ...
            throw new RuntimeException(String.format("Parser %s has no %s property", parserName, paramName));
        }
        throw new RuntimeException(String.format("Parser %s has no %s property", parserName, paramName));
    }
