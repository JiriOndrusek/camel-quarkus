...
class Langchain4jIngestProcessor {
     ...
     * can be checked: a builder-declared pipeline composes its URI at startup, and Camel reports
     * the missing component then. Component services are REGISTRY-destination, so they are read
     * from the application archives directly — they never appear among the DISCOVERY
        ...
        for (Map.Entry<String, IngestBuildTimeConfig.PipelineBuildTimeConfig> entry : config.pipelines().entrySet()) {
            ...
            if (!components.contains(scheme)) {
                validationErrors.produce(new ValidationErrorBuildItem(new ConfigurationException(
                        "Ingestion pipeline '" + entry.getKey() + "' consumes from '" + uri
                                + "', but the Camel component '" + scheme + "' is not on the classpath.\n"
                                + "Add it:  ./mvnw quarkus:add-extension -Dextensions=camel-quarkus-" + scheme)));
            }
        }
    }
