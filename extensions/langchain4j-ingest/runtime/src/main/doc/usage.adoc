...
[NOTE]
====
This experimental extension keeps no record of what it wrote: an edited document re-ingests on top of its old segments, and removing a document from the source removes nothing from the store. Replace and delete arrive with the synchronising engine in a later release. What is remembered is which documents were already ingested — the duplicate register below.
====

=== The duplicate register

A directory pipeline registers what it ingested, keyed on the file's path, modification time and size: unchanged files are skipped, edited files re-ingest (their previous segments remain, see the note above). The default register is in-memory, holds 100,000 keys and is lost on restart. `source.idempotent-repository` names an `IdempotentRepository` bean to use instead:
...
`#class:` beans are created reflectively. The extension registers the two repository classes shown here for native mode; a custom class needs `@RegisterForReflection` or a `quarkus.camel.native.reflection.include-patterns` entry.
