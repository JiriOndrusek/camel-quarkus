...
`source.includes` and `source.excludes` narrow a directory pipeline to matching files — comma-separated Ant path patterns matched against the path relative to `source.directory`, exclusion winning over inclusion:

[source,properties]
----
quarkus.camel.langchain4j.ingest.products.source.includes=*.txt,**/*.txt
quarkus.camel.langchain4j.ingest.products.source.excludes=**/drafts/**
----

A pipeline consuming through `source.uri` filters with its consumer's own options instead.
...
`tika` extracts plain text in-process with Apache Tika. It needs the `camel-quarkus-tika` extension, plus the Tika parser module for each format the pipeline should read — `org.apache.tika:tika-parser-pdf-module` for PDF, say. A format without its module parses to nothing and the document is skipped as empty (logged as a warning); so is a scanned, image-only PDF, since Tika does no OCR here. On a directory pipeline an empty parse still commits the file's key in the duplicate register, so the file is not retried until it changes — after adding a missing parser module, force a re-ingest (restart with the default register, or clear the persistent one). A consumer-fed pipeline releases the claim of an empty delivery instead.

`docling` converts documents to structure-preserving markdown through a https://github.com/docling-project/docling-serve[Docling Serve] instance, OCR included. It needs the `camel-quarkus-docling` extension and the instance's address in `camel.component.docling.docling-serve-url`.

A missing parser extension fails the build naming the artifact, the same way a missing consumer component does. The Java twin is `IngestPipeline.parser("tika")`, and a consumer-fed pipeline parses the same way — the payload the consumer delivers goes to the parser first.
...
With xref:reference/extensions/microprofile-health.adoc[`camel-quarkus-microprofile-health`] present, a readiness check named `camel-quarkus-langchain4j-ingest` reports each pipeline under `/q/health/ready`: ready while every pipeline's route is started, down naming the stopped ones. Without a health extension the check is simply not registered.

=== Metrics

With `io.quarkus:quarkus-micrometer` present (plus a registry such as `quarkus-micrometer-registry-prometheus` to expose it), every pipeline counts its work into Micrometer meters tagged with the pipeline name: `cq.ingest.documents` and `cq.ingest.segments` for what was written, `cq.ingest.empty` for blank documents, `cq.ingest.skipped` for duplicate deliveries answered from the register, and `cq.ingest.failures` for ingestions that threw. Without Micrometer the counters cost nothing — the extension does not depend on it. In dev mode the same counters appear on the extension's Dev UI card, one row per pipeline next to its source and configuration.
