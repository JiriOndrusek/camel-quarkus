...
public class IngestRoutes extends RouteBuilder {
    ...
    private void configureFileSource(String name, IngestRunTimeConfig.PipelineRunTimeConfig runtime,
        ...
        // it - the truncation would be permanent, since idempotent keys on the path
    ...
    /** Picking one silently would bind a pipeline to whichever bean happened to be discovered first. */
    private <T> T single(String name, Instance<T> candidates, String what, String property) {
        var beans = StreamSupport.stream(candidates.spliterator(), false).collect(Collectors.toList());
        if (beans.size() != 1) {
            throw new IllegalStateException("Ingestion pipeline '" + name + "' found " + beans.size() + " "
